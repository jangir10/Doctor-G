...
const Appointment = () => {
  ...
  const fetchDocInfo = async ()=>{
    ...
    const docInfo = doctors.find(doc => doc._id === docId.docID)
    setDocInfo(docInfo)
  }
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <div className='flex items-center gap-2 text-sm mt-1 text-gray-600'>
            ...
            <button className='py-0.5 px-2 border text-xs rounded-full cursor-default'>{docInfo.experience}</button>
          </div>
          {/* -----Doctor About ------ */}
          <div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll scrollbar-hidden mt-4'>
            {docSlots.length && docSlots.map((item,index)=>(
              item[0] && <div onClick={()=>{setSlotIndex(index)}} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-primary text-white':'border border-gray-300'}`} key={index}>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll scrollbar-hidden mt-4'>
